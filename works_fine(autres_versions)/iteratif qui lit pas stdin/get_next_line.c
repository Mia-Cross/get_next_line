...
/*   Updated: 2019/11/19 15:55:01 by lemarabe         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int		get_next_line(int fd, char **line)
{
	...
	*line = ft_strdup("");
	...
	{
		if (process_stock(&stock[fd], line))
			return (1);
	}
	ret = read_next_line(fd, line, &stock[fd]);
	if (ret == -1)
		return (-1);
	if (ret < BUFFER_SIZE && !(stock[fd]) && fd != 0)
		return (0);
	else if (ret == BUFFER_SIZE || stock[fd])
		return (1);
	else if (fd == 0)
